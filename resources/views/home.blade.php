...
@section('content')
    ...
    @if(isset($heroProducts) && $heroProducts->count() > 0)
        <section class="bg-black text-white relative h-[500px] md:h-[600px] flex items-center"
            style="overflow: hidden; position: relative; isolation: isolate;" x-data="{
                                     currentSlide: 0,
            ...
            <div class="absolute inset-0 z-0"
                style="overflow: hidden; position: absolute; top: 0; left: 0; right: 0; bottom: 0; contain: layout style paint;">
                @foreach($heroProducts as $index => $product)
                    <div x-show="currentSlide === {{ $index }}" x-cloak x-transition:enter="transition ease-out duration-1000"
                        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-30"
                        x-transition:leave="transition ease-in duration-1000" x-transition:leave-start="opacity-30"
            ...
            <!-- Boutons navigation -->
                    <div class="absolute top-1/2 left-4 transform -translate-y-1/2 z-30 hidden md:block">
                        <button @click="prev()"
                            class="bg-white/20 hover:bg-white/30 text-white rounded-full p-3 transition shadow-lg">
                            <i class="fas fa-chevron-left text-xl"></i>
                        </button>
                    </div>
                    <div class="absolute top-1/2 right-4 transform -translate-y-1/2 z-30 hidden md:block">
                        <button @click="next()"
                            class="bg-white/20 hover:bg-white/30 text-white rounded-full p-3 transition shadow-lg">
                        ...
                        <div class="relative h-full min-h-[400px]">
                            @foreach($heroProducts as $index => $product)
                                <div x-show="currentSlide === {{ $index }}" x-transition:enter="transition ease-out duration-500"
                                    ...
                                    class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center absolute inset-0 md:px-16"
                                    :class="currentSlide === {{ $index }} ? 'z-10' : 'z-0 pointer-events-none'">
                                    <!-- Contenu texte -->
                                    <div class="z-10 relative">
                                        ...
                                        <div class="mb-6">
                                            <a href="{{ route('products.show', $product->slug) }}" class="block">
                                                <h3
                                                    class="text-xl md:text-2xl lg:text-3xl font-semibold mb-4 hover:text-orange-500 transition line-clamp-2">
                                                    {{ $product->name }}
                                                </h3>
                                            </a>
                                    ...
                                    @if($product->image)
                                        <div class="z-10 relative flex justify-center items-center h-full">
                                            <div class="relative w-full max-w-lg">
                                                <div class="absolute inset-0 bg-green-500 opacity-10 rounded-full blur-3xl transform rotate-12">
                                                </div>
                                                <a href="{{ route('products.show', $product->slug) }}" class="block relative aspect-square">
                                                    <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}"
                                                        class="w-full h-full object-contain rounded-lg shadow-2xl transform hover:scale-105 transition duration-300">
                                                </a>
                                            </div>
                                        </div>
                                    @endif
                    ...
                    <div class="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex gap-2 z-20">
                        @foreach($heroProducts as $index => $product)
                            <button @click="goToSlide({{ $index }})" class="w-3 h-3 rounded-full transition"
                ...
                <div class="container mx-auto px-4">
                    <div class="text-center">
                        ...
                        <a href="{{ route('products.index') }}"
                            class="bg-white text-orange-600 px-6 py-2 rounded-lg text-sm font-medium hover:bg-gray-100 transition">
        ...
        @if(isset($flashSaleProducts) && $flashSaleProducts->count() > 0)
            <section class="bg-white py-6 container mx-auto px-4">
                <div class="bg-red-600 text-white px-4 py-3 rounded-t-lg flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('products.index', ['filter' => 'on_sale']) }}"
                ...
                <div class="relative" x-data="{
                    ...
                    <!-- Bouton précédent -->
                    <button @click="scroll(-1)" :class="canScrollLeft ? 'opacity-100' : 'opacity-0 pointer-events-none'"
                    ...
                    <div x-ref="scrollContainer" @scroll="checkScroll()"
                        class="flex gap-4 overflow-x-auto scrollbar-hide pb-4 px-10"
                        style="scrollbar-width: none; -ms-overflow-style: none;">
                        @foreach($flashSaleProducts as $product)
                            <div
                                class="flex-shrink-0 w-48 md:w-56 bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition">
                                <a href="{{ route('products.show', $product->slug) }}" class="block">
                                    <!-- Badge de réduction -->
                                    @if($product->is_on_sale && $product->sale_price)
                                        <div class="relative">
                    ...
                    <!-- Bouton suivant -->
                    <button @click="scroll(1)" :class="canScrollRight ? 'opacity-100' : 'opacity-0 pointer-events-none'"
        ...
        @if(isset($categories) && $categories->count() > 0)
            <section class="py-8 container mx-auto px-4">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('products.index') }}" class="text-sm text-orange-600 hover:text-orange-700 font-medium">Voir
                        tout</a>
                </div>
                <div class="relative" x-data="{
                                scrollPosition: 0,
                    ...
                    <div x-ref="categorySlider" @scroll="checkScroll()" class="flex gap-3 overflow-x-auto scrollbar-hide pb-2 px-6"
                        style="scrollbar-width: none; -ms-overflow-style: none;">
                        @foreach($categories as $category)
                            <a href="{{ route('category.show', $category->slug) }}" class="flex-shrink-0">
                                <div class="w-36 bg-white rounded-2xl shadow p-4 text-center hover:shadow-lg transition">
                                    <div
                                        class="w-12 h-12 bg-orange-100 rounded-full mx-auto mb-3 flex items-center justify-center group-hover:bg-orange-500 transition">
        ...
        @if(isset($randomProducts) && $randomProducts->count() > 0)
            <section class="py-6 container mx-auto px-4">
                ...
                <div class="relative" x-data="{
                    ...
                    <div x-ref="randomSlider" @scroll="checkScroll()" class="flex gap-4 overflow-x-auto scrollbar-hide pb-4 px-10"
                        style="scrollbar-width: none; -ms-overflow-style: none;">
