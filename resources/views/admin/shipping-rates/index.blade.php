    ...
    <div class="space-y-4">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('admin.shipping-rates.create') }}"
                class="bg-orange-500 text-white px-4 py-2 text-sm rounded-lg hover:bg-orange-600 font-semibold">
                    ...
                    @forelse($shippingRates as $rate)
                        <tr>
                            <td class="px-4 py-3">
                                ...
                                @if($rate->description)
                                    <div class="text-xs text-gray-500">{{ \Illuminate\Support\Str::limit($rate->description, 50) }}
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <span
                                    class="px-2 py-0.5 text-xs rounded-full {{ $rate->is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
