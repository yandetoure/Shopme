            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" id="price" name="price" value="{{ old('price', $shippingRate->price) }}" step="0.01" min="0" required
                               class="w-full px-3 py-1.5 text-sm border rounded-lg focus:ring-2 focus:ring-orange-500">
                    </div>

                    <div>
                        <label for="estimated_days" class="block text-xs font-medium text-gray-700 mb-1">Jours estimés</label>
                        <input type="number" id="estimated_days" name="estimated_days" value="{{ old('estimated_days', $shippingRate->estimated_days) }}" min="1"
                               class="w-full px-3 py-1.5 text-sm border rounded-lg focus:ring-2 focus:ring-orange-500">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" id="min_order_amount" name="min_order_amount" value="{{ old('min_order_amount', $shippingRate->min_order_amount) }}" step="0.01" min="0"
                               class="w-full px-3 py-1.5 text-sm border rounded-lg focus:ring-2 focus:ring-orange-500">
                    </div>

                    <div>
                        <label for="max_order_amount" class="block text-xs font-medium text-gray-700 mb-1">Montant maximum (FCFA)</label>
                        <input type="number" id="max_order_amount" name="max_order_amount" value="{{ old('max_order_amount', $shippingRate->max_order_amount) }}" step="0.01" min="0"
                               class="w-full px-3 py-1.5 text-sm border rounded-lg focus:ring-2 focus:ring-orange-500">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $shippingRate->sort_order) }}" min="0"
                           class="w-full px-3 py-1.5 text-sm border rounded-lg focus:ring-2 focus:ring-orange-500">
                </div>

                <div class="flex items-center">
                    <input type="checkbox" id="is_free" name="is_free" value="1" {{ old('is_free', $shippingRate->is_free) ? 'checked' : '' }}
                           class="mr-2">
                    <label for="is_free" class="text-xs font-medium text-gray-700">Livraison gratuite</label>
                </div>

                <div class="flex items-center">
                    <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $shippingRate->is_active) ? 'checked' : '' }}
                           class="mr-2">
